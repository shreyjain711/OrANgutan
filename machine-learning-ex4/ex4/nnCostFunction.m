...
error = (lambda/(2*m)) * (sum(sum((Theta1(:,2:end).^2))) + sum(sum((Theta2(:,2:end).^2))));
J = J + error;
...
for t = 1:m
    ...
    a3_curr_row = a3(t,:);
    y_curr_row = y(t,:);
    d3 = a3_curr_row - y_curr_row;
    d2 = Theta2'*d3' .* sigmoidGradient([1;Theta1 * a1_curr_row']);
    del1 = del1 + d2(2:end)*a1_curr_row;
    del2 = del2 + d3' * a2_curr_row;
end    
